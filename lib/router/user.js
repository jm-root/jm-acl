...
module.exports = function (service, opts) {
    ...
    async.waterfall([
      function (cb) { // 获取用户创建的角色
        service.role.find({creator: userId}, function (err, doc) {
          if (err) return cb(err, ERR.FA_NOTFOUND)
          cb(null, doc)
        })
      },
  ...
  router
    ...
    .add('/:id/roles', 'put', function (opts, cb, next) {
      var roles = opts.data.roles || opts.data.role
      roles = Array.isArray(roles) ? roles : roles.toString().split(',')
      var updateData = {$addToSet: {roles: {$each: roles}}}
      opts.data.nick && (updateData.nick = opts.data.nick)
      opts.data.creator && (updateData.creator = opts.data.creator)
      opts.data.tags && (updateData.tags = opts.data.tags)
      service.addUserRoles(opts.params.id, roles, function (err, doc) {
        doc = {ret: 0}
        if (!err) doc = {ret: 1}
        if (!doc.ret) return cb(err, doc)
        user.update({_id: opts.params.id}, updateData, {upsert: true, setDefaultsOnInsert: true}, function (err, upd) {
          cb(err, doc)
        })
      })
    })
    .add('/:id/roles', 'delete', function (opts, cb, next) {
      var roles = opts.data.roles || opts.data.role
      roles = Array.isArray(roles) ? roles : roles.toString().split(',')
      service.removeUserRoles(opts.params.id, roles, function (err, doc) {
        doc = {ret: 0}
        if (!err) doc = {ret: 1}
        if (!doc.ret) return cb(err, doc)
        user.update({_id: opts.params.id}, {$pullAll: {roles: roles}}, function (err, upd) {
          cb(err, doc)
        })
      })
    })
  ...
  user.routes.before_list = function (opts, cb, next) {
    ...
    if (opts.data.keyword) {
      ...
      if (ObjectId.isValid(keyword)) {
        opts.conditions.$or.push({_id: keyword})
      }
    }
  ...
  user.routes.after_update = function (opts, cb, next) {
    ...
    async.waterfall([
      ...
      function (cb) { // 添加新的角色
        if (!opts.data.roles.length) return cb()
