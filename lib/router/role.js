...
module.exports = function (service, opts) {
  ...
  var init = function (opts, cb) {
    ...
    role.createOrUpdate(rows, function (err, ret) {
  ...
  var roleResources = function (opts, cb, next) {
    ...
    if (permissions) { permissions = Array.isArray(permissions) ? permissions : permissions.toString().split(',') }
  ...
  var _roleUsers = function (opts, cb) {
    var id = opts.params.id
    service.acl.roleUsers(id, function (err, doc) {
      if (err) {
        logger.error(err)
        return cb(err, null)
      }
  ...
  var _remove = function (opts, cb) {
    ...
    var permissions = opts.data.permissions
    if (permissions) {
      service.acl.removeAllow(roles, resources, permissions, cb)
    } else {
      service.acl.removeAllow(roles, resources, cb)
    }
  }
  // 给某些角色增加某些资源的某些权限
  var _setResourcePermission = function (opts, cb) {
    ...
    async.parallel([
      ...
      function (cb) {
        service.acl.allow([{roles: roles, allows: allows}], function (err, doc) {
          err ? cb(err) : cb()
        })
      }
  ...
  role.routes.after_create = function (opts, cb, next) {
    ...
    if (opts.doc.parents) {
      ...
      service.acl.addRoleParents(opts.doc.code, opts.doc.parents, function (err, doc) {
        if (err) {
          opts.err = err
        }
        next()
  ...
  role.routes.before_update = function (opts, cb, next) {
    if (opts.data.parents) {
      ...
      service.acl.removeRoleParents(code, function (err, doc) {
        if (err) {
          opts.err = err
        }
        next()
  ...
  role.routes.after_update = function (opts, cb, next) {
    ...
    if (opts.data.parents) {
      var code = role.codeById(opts.params.id)
      service.acl.addRoleParents(code, opts.data.parents, function (err, doc) {
        if (err) {
          opts.err = err
        }
        next()
    ...
    async.waterfall([
      ...
      function (roleCode, cb) { // 移除角色关系表(包含子角色)
        async.each(roleCode, function (code, callback) {
          service.acl.removeRole(code, function (err, doc) {
      ...
      function (roleCode, cb) { // 获取拥有此角色的用户
        var userIds = []
        async.each(roleCode, function (code, callback) {
          service.acl.roleUsers(code, function (err, docs) {
            if (err) return callback(err)
      ...
      function (userIds, cb) { // 移除用户角色关系
        async.each(userIds, function (id, callback) {
          service.acl.removeUserRoles(id, roleCode, function (err, doc) {
            if (err) return callback(err)
            callback()
          })
        }, function (err) {
