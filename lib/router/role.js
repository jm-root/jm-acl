...
module.exports = function (service, opts) {
    ...
    var _remove=function (opts, cb) {
        ...
        var permissions=opts.data.permissions||null;
        acl.removeAllow(roles, resources, permissions,cb);
    };
    //给某些角色增加某些资源的某些权限
    var _setResourcePermission=function (opts, cb) {
        var roles=opts.data.roles;
        var allows=opts.data.allows;
        async.async.series({
            flag1:function(done){
                _remove(opts, function (error,doc) {
                    if(!error){
                        done(null,doc);
                    }else {
                        done(error, null);
            ...
            flag2:function(done){
                acl.allow([{roles:roles,allows: allows}],function (err,doc) {
                    if(!error){
                        done(null,doc);
                    }else {
                        done(error, null);
