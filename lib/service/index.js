...
module.exports = function (opts) {
    ...
    var o = {
        ...
        hasSuperRole: function(user, cb) {
            this.hasRole(user, superRole, cb);
        },
        ...
        isAllowed: function (user, resource, permissions, cb) {
            ...
            self.areAnyRolesAllowed(guestRole, resource, permissions, function(err, doc){
        ...
        addUserRoles: function(user, roles, cb) {
            cb || (cb=cb_default);
            if(!user || !roles) {
                return cb(mkError(ERR.FA_PARAMS), ERR.FA_PARAMS);
            }
        ...
        userRoles: function(user, cb) {
            ...
            self.hasSuperRole(user, function (err, doc) {
                ...
                if(doc){
                    cb(null, Object.keys(self.role.nodesByCode || {}));
                }else{
                    self.acl.userRoles(user, cb);
                }
            });
        },
