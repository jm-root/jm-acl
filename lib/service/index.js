...
module.exports = function (opts) {
  ...
  var defaultAllow = opts.defaultAllow || false
  ...
  var o = {
    ...
    /**
         * 检测是否有权限访问，处理过程如下
         * 1，如果请求的资源没有登记到acl，defaultAllow=true时允许访问
         * 2，如果请求的所有权限都没有登记到acl，defaultAllow=true时允许访问
    ...
    isAllowed: function (user, resource, permissions, cb) {
      ...
      if (this.resource.loaded) {
        // 如果资源没有登记到acl，允许访问
        if (!resource || !this.resource.idByCode(resource)) return Promise.resolve(self.defaultAllow).asCallback(cb)
        ...
        if (!bExistPermission) return Promise.resolve(self.defaultAllow).asCallback(cb)
      }
      ...
      self.hasSuperRole(user, function (err, doc) {
        ...
        if (doc) {
          ...
          for (var key in self.resource.nodesByCode) {
            if (!resource || pick.indexOf(key) != -1) { ret[key] = self.resource.nodesByCode[key].permissions }
          }
          cb(null, ret)
        } else {
          async.waterfall([
        ...
        if (doc) {
          ...
          self.userResources(user, function (err, docs) {
            for (var code in docs) {
              if (nodesByCode[code]) { nodesByCode[code].permissions = docs[code].permissions }
            }
            cb(null, {rows: self.resource.roots})
          })
        }
  ...
  if (db) {
    db.on('open', function () {
      var prefix = 'acl_'
      if (tableNamePrefix) prefix = '_' + tableNamePrefix
      o.acl = new Acl(new Acl.mongodbBackend(db.db, prefix))
    })
  }
